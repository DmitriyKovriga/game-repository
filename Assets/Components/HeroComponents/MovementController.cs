...
namespace hero
{
    public class MovementController : MonoBehaviour
    {
        ...
        public void setHorizonalInput(InputAction.CallbackContext context) //получаем инпут по х и у, передается он в виде двух значение вектора 2д
        {
        ...
        public void OnInteract(InputAction.CallbackContext context)
        {
            if (context.canceled)
            {
                var hit = Physics2D.OverlapCircleNonAlloc(   //данный метод возвращает нам размер массива (количествой найденных объектов с нужным слоем),
                    transform.position, //позиция отрисовки круга
                    1, //радиус круга
                ...
                for (int i = 0; i < hit; i++)
                {
                    ...
                    {
                        Debug.Log("Выполнился скрипт который запускает интеракцию, она произошла с объектом " + interactable);
